...
|更新异常|学院更换院长后，系统必须修改与该学院学生有关的每一个元组|
|插入异常|一个学院刚成立，尚无学生，则这个学院及其院长的信息就无法存入数据库|
|删除异常|例如，某个学院的学生全部毕业了，在删除该学院学生信息的同时，也把这个学院的信息全部删除了。|
...
3. 多表关系运算
   ...
   2. \\(\theta\\) 连接。关系代数中的所有连接都是笛卡尔积经过选择或投影运算后的结果，即$$R \bowtie_{R.A \, \theta \, S.B} S = \sigma_{R.A \, \theta \, S.B} (R \times S).$$
   ...
   3. 外连接。首先包含等值连接的结果；对于指定表中没匹配上的行，也要保留，但零一部分的列**补为 `NULL`**（空值）。
